...
function GitHubPopup() {
    ...
    return (
        <div style={{
            ...
            <style>{`@keyframes popIn { from { opacity:0; transform:translateY(-50%) scale(0.88); } to { opacity:1; transform:translateY(-50%) scale(1); } }`}</style>
            {data ? (<>
    ...
    return (
        <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px" }}>
                ...
                <div>
                    <div style={{ color: "#fff", fontWeight: 700, fontSize: "13px", fontFamily: "monospace" }}>
                        Varun Rana
                    </div>
                    <div style={{ color: "#e8d44d", fontSize: "11px", fontFamily: "monospace" }}>
                        LinkedIn
                    </div>
                </div>
            </div>
...
export default function Hero({ setHovered, currentYRef }) {
    ...
    useEffect(() => {
        ...
        const animate = () => {
            ...
            // HEY I'M: grey #5f5f5f → near black #1c1c1c
            const gFrom = 95, gTo = 28
            const gVal = Math.round(gFrom + (gTo - gFrom) * p)
            if (heyTextRef.current) {
                heyTextRef.current.style.color = `rgb(${gVal},${gVal},${gVal})`
            }

            // VARUN: yellow #e8d44d (232,212,77) → dull dark #3a3510 (58,53,16)
            ...
            if (varunTextRef.current) {
                varunTextRef.current.style.color = `rgb(${rV},${gV},${bV})`
            }
